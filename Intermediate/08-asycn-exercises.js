...
//             thirdPromise().
//             then(()=>{
//                 console.log("All primises have been done");
//             });
//         });
//     });
// };
...
function checkBalance() {
    return new Promise((resolve) => {
        setTimeout(() => {
            console.log(`Avaliable balance: 500$`);
            resolve(500);
        }, 1000);
    });
}

function withdrawMoney(amount, currentBalance) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            console.log(`Withdrawing ${amount}$...`);
            if (currentBalance >= amount) {
                currentBalance -= amount;
                console.log(`Succesful operation, remaining balance: ${currentBalance}$`);
                resolve(currentBalance);
            } else {
                reject(`Error: Insufficent founds`);
            }
        }, 2000);
    });
}
